...
class ADAINHourglass(nn.Module):
    def __init__(self, image_nc, pose_nc, ngf, img_f, encoder_layers, decoder_layers, nonlinearity, use_spect, out_planes):
        ...
        self.out_conv = OutConv(self.output_nc, out_planes)

    def forward(self, x, z):
        return self.out_conv(self.decoder(self.encoder(x, z), z))                 

class SPADEHourglass(nn.Module):
    def __init__(self, image_nc, pose_nc, ngf, img_f, encoder_layers, decoder_layers, nonlinearity, use_spect, opt=None):
        super(SPADEHourglass, self).__init__()
        self.encoder = SPADEEncoder(image_nc, pose_nc, ngf, img_f, encoder_layers, nonlinearity, use_spect, opt=opt)
        self.decoder = SPADEDecoder(pose_nc, ngf, img_f, encoder_layers, decoder_layers, True, nonlinearity, use_spect, opt=opt)
        self.output_nc = self.decoder.output_nc
...
class SPADEDecoder(nn.Module):
    ...
    def __init__(self, pose_nc, ngf, img_f, encoder_layers, decoder_layers, skip_connect=True,
                 nonlinearity=nn.LeakyReLU(), use_spect=False, opt=None):
        ...
        for i in range(encoder_layers-decoder_layers, encoder_layers)[::-1]:
            ...
            model = SPADEResnetBlock(in_channels, out_channels, opt, fs=pose_nc, norm_G=opt.GENERATOR.SPADE.norm_G,
...
class SPADEEncoder(nn.Module):
    def __init__(self, image_nc, pose_nc, ngf, img_f, layers, nonlinearity=nn.LeakyReLU(), use_spect=False, opt=None, fs=None):
        ...
        for i in range(layers):
            ...
            model = SPADEResnetBlock(in_channels, out_channels, opt, fs=pose_nc, downsample=True,
                                     norm_G=opt.GENERATOR.SPADE.norm_G, learned_shortcut=True)
...
class ADAINDecoder(nn.Module):
    ...
    def forward(self, x, z):
        ...
        for i in range(self.encoder_layers-self.decoder_layers, self.encoder_layers)[::-1]:
            ...
            # print(x.pop().shape)
            res = x.pop()
            diffY = res.size()[2] - out.size()[2]
            diffX = res.size()[3] - out.size()[3]
            out = F.pad(out, [diffX // 2, diffX - diffX // 2,
                            diffY // 2, diffY - diffY // 2])
            # print(out.shape)
            # print(res.shape)
            out = torch.cat([out, res], 1) if self.skip_connect else out
        return out
