...
class OrbiterDatasetRef(Dataset):
  def __init__(self, dataset, ref_img, scale, dmin,
      dmax, invz, transform=None,
      render_style='', offset=200, cv2resize=False):
    self.scale = scale
  ...
  def __getitem__(self, idx):
    ...
    img = self.fromCache(self.dataset + "/" + self.imgs[idx]['path'], self.scale)
    img_ref = self.fromCache(self.dataset + "/" + self.sfm.ref_img['path'], self.scale)
    # ref_input = cv2.Canny(np.uint8(img_ref*255), 100, 200)
    # ref_input = cv2.Canny(np.uint8(img_ref*255), 20, 50)
    # ref_input = cv2.Canny(np.uint8(img_ref*255), 50, 100)
    # cv2.imwrite("test.jpg", ref_input) 
    # ref_input = np.random.rand(img.shape[0], img.shape[1])
    # cv2.imwrite("test_1.jpg", ref_input*255) 

    ref_input = io.imread("test_1.jpg").astype(np.float32)
...
# Load an image and its corresponding keypoints skelenton at the same time
class OrbiterDatasetMulti(Dataset):
  def __init__(self, dataset, ref_img, scale, dmin,
      dmax, invz, transform=None,
      render_style='', offset=200, cv2resize=False, mask_weight=0, use_gaussian=False, 
      render_keypoints_path='keypoints_2d'):
    self.scale = scale
    self.dataset = dataset
    self.transform = transform
    self.sfm = SfMData(dataset,
    ...
    self.use_all_static = False 
    # can add more data for static scene
    if self.use_all_static:
      # ind_s = len(self.imgs)
      ref_img_tmp = self.sfm.ref_img
      file_list = glob.glob(self.dataset + "/images_drive/*")
      for file_name in file_list:
        seg = file_name.split("/")
        file_name = seg[-2] + "/" + seg[-1] 
        ref_img_tmp['path'] = file_name
        self.imgs.append(ref_img_tmp)
    ...
    self.render_keypoints_path = render_keypoints_path


  def fromCache(self, img_path, scale, pos=False, use_gaussian=False):
    p = (img_path, scale)
    if p in self.cache:
      return self.cache[p]
  ...
  def __getitem__(self, idx):
    ...
    # img_ref = self.fromCache(self.dataset + "/" + self.sfm.ref_img['path'], self.scale)
    # ref_input  
    ref_name = self.imgs[idx]['path'].split("/")[-1].split(".")[0]
    camera_index = int(ref_name[-1])
    ref_name = ref_name[:-1] + "0" 
    ref_input = self.fromCache(self.dataset + f"/{self.render_keypoints_path}/" + ref_name + ".png", self.scale, pos=True)
    if self.use_gaussian:
      feature_path = self.dataset + "/keypoints_tensor_1/" + ref_name + ".npy"
      features = self.fromCache(feature_path, self.scale, use_gaussian=self.use_gaussian)
      features = features / 255.0
      ref_input = np.concatenate([ref_input, features], axis=2)

    mask_input = 0 
    if self.mask_weight > 0:
      mask_name = self.imgs[idx]['path'].split("/")[-1]
      mask_name = mask_name + ".png"
      mask_input = self.fromCache(self.dataset + "/masks/" + mask_name, self.scale)
      mask_input = np.transpose(mask_input, [2, 0, 1]).astype(np.float32)
      if np.max(mask_input) > 1:
        mask_input /= 255.0
      mask_input[mask_input<0.95] = self.mask_weight
      
    img = np.transpose(img, [2, 0, 1]).astype(np.float32)
    if np.max(img) > 1:
      img /= 255.0

    ref_input = np.transpose(ref_input, [2, 0, 1]).astype(np.float32)
    if np.max(ref_input) > 1:
      ref_input /= 255.0
...
def evaluation(model, dataset, dataloader, ray, model_dir_nolp, webpath = 'runs/html/', write_csv = True):
  # outputFile = "/".join(model_dir_nolp.split("/")[1:])
  # outputFile = "/".join(model_dir_nolp.split("/")[1:])
  print(model_dir_nolp)
  outputFile = model_dir_nolp.split("/")[-2]
...
def render_video(model, dataset, ray, video_path = 'runs/video/', render_type='default', dataloader = None):
  ...
  # os.system('ffmpeg -r 15 -i {}/frame_%04d.png -c:v libx264 -crf 12 -pix_fmt yuv420p -vf pad="width=ceil(iw/2)*2:height=ceil(ih/2)*2" {}/video.mp4 < /dev/null'.format(video_path, video_path))
  os.system('ffmpeg -r 15 -i {}/frame_%04d.png -c:v libx264 -crf 12 -pix_fmt yuv420p -vf pad="width=ceil(iw/2)*2:height=ceil(ih/2)*2" {}/video.mp4 < /dev/null'.format(video_path, video_path))
  print("output video at {}/video.mp4".format(video_path))


def render_video_new(model, dataset, ray, video_path = 'runs/video/', render_type='default', dataloader = None):
  print("render_video: {}".format(render_type))
  ...
  total_frame = len(render_poses)
  i=0
  bullet_points=[33, 63, 90, 120]
  for pose_id in render_poses:
    # pose = render_poses[pose_id]
    if render_type == "render_fixed_view":
      pose = dataset.sfm.ref_img
    else:
      pose = render_poses[pose_id]

    if render_type == "bullet_time":
      if i < bullet_points[0]:
        idx = (i * 5) % len(dataset)
      elif i >=bullet_points[0] and i < bullet_points[1]:
        idx = (bullet_points[0] * 5) % len(dataset)
      elif i >=bullet_points[1] and i < bullet_points[2]:
        idx = ((i-bullet_points[0]) * 5) % len(dataset)
      elif i >=bullet_points[2] and i < bullet_points[3]:
        idx = ((bullet_points[0]+bullet_points[2]-bullet_points[1]) * 5) % len(dataset)
      elif i >=bullet_points[3]:
        idx = ((i-(bullet_points[0]+bullet_points[2]-bullet_points[1])) * 5) % len(dataset)
    else:
      idx = (i * 5) % len(dataset)
    # start = timer()
    feature = dataset.__getitem__(idx)
    # end = timer()
    # print(f"get_item:{end - start}")
    i = i+1
    # require to pad batch dimension to compatible with torch
    ...
    if not render_type == 'nearest':
      predict_image = patch_render(model, dataset.sfm, feature, ray)
      # predict_image = patch_render(model, dataset.sfm, dataset.__getitem__(i*5 % len(dataset)), ray)
      predict_image = predict_image.permute(0, 2, 3, 1).cpu().detach().numpy()[0]
    else:
      closet = [(pt.mean((feature['center'][0] - feature_train['center'][0])**2), feature_train['image']) for feature_train in dataloader]
      closet.sort(key = lambda x: x[0])
  ...
  if os.path.exists(video_filepath):
    os.remove(video_filepath)
  os.system('ffmpeg -r 10 -i {}/frame_%04d.png -c:v libx264 -crf 12 -pix_fmt yuv420p -vf pad="width=ceil(iw/2)*2:height=ceil(ih/2)*2" {}/video.mp4 < /dev/null'.format(video_path, video_path))
  print("output video at {}/video.mp4".format(video_path))
...
def patch_render(model, sfm, feature, ray):
  ...
  with pt.no_grad():
    ...
    for i in range(num):
      ...
      # start = timer()
      output, _, _ = model(sfm, feature, output_shape, selection)
...
def measurement(model, sfm, dataloader, ray, write_path = ''):
  ...
  for i, feature in enumerate(dataloader):
    ...
    if write_path != '':
      io.imsave(os.path.join(write_path, f'{i:05d}_ours.png'), (predict_image * 255).astype(np.uint8))
      io.imsave(os.path.join(write_path, f'{i:05d}_gt.png'), (gt * 255).astype(np.uint8))
...
# Sel all pixels
def Asel(shape):
  sel = pt.tensor(np.arange(shape[0] * shape[1]))
  return sel