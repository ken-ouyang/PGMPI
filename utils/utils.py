...
import sys
import json
from torch.utils.data import SubsetRandomSampler,DataLoader, Subset, Sampler, SequentialSampler
...
def prepareDataloaders(dataset, dpath, random_split=False, train_ratio=1, num_workers=8, ddp=False, world_size=1, rank=0):
  if random_split:
    ...
    if os.path.exists(os.path.join(dpath,'poses_bounds.npy')):
      ...
      indices_total = list(range(len(dataset.imgs)))
      indices_val = indices_total[::16]
      print(indices_val)
      # indices_val = indices_total[::300]
    ...
    if not ddp:
      ...
      val_set = Subset(dataset, indices_val) 
      ...
      dataloader_val = DataLoader(val_set, batch_size=1, sampler = sampler_val, num_workers = num_workers, shuffle=False)
    else:
      train_set = Subset(dataset, indices_train) 
      sampler_train = DistributedSampler(train_set, num_replicas=world_size, rank=rank) 
      dataloader_train = DataLoader(train_set, batch_size=1, sampler = sampler_train, num_workers = 0)
      val_set = Subset(dataset, indices_val) 
      sampler_val = DistributedSampler(val_set, num_replicas=world_size, rank=rank, shuffle=False) 
      dataloader_val = DataLoader(val_set, batch_size=1, sampler = sampler_val, num_workers = 0)
    print('TRAINING IMAGES: {}'.format(len(dataloader_train)))
    print('VALIDATE IMAGES: {}'.format(len(dataloader_val)))
